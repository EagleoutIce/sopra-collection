...
\solLoadLanguage{latex,bash,rubberduck}
...
\begin{center}
    Aktuell sind noch einige Dinge wie Optionen und weitere Umgebungen geplant. % TODO: autogobble?
\end{center}
...
\end{plainlatex}
    ...
    nicht gewünscht oder möglich sein, so \emph{muss} \cmdref{solLanguageSearchPath} angepasst werden! Es steht jedem Frei, das Laden der Sprachen mittels \jmark[\T{noloadlangs}]{mrk:noloadlangs} zu deaktivieren und seine eigenen Sprachdefinitionen
...
In Version \thesolversion{} (\cmdref{thesolversion}) gibt es keine weiteren
Besonderheiten.
...
\subsection{Akzeptierte Parameter}
    ...
    geschrieben. So wird implizit: % TODO:
\begin{plainlatex}
...
\end{plainlatex}
    ...
    \begin{argument}{loadlangs}{noloadlangs}
        Lädt mittels \cmdref{solLoadLanguage} die Sprachen \T{java}, \T{xml} und \T{json} (siehe \jmark[Abschnitt \ref{sec:vordefinierteSprachen} (Vordefinierte Sprachen)]{sec:vordefinierteSprachen} für mehr Informationen). Dies kann durch \T{noloadlangs} deaktiviert werden. \notetext{Hinweis: Die Sprachen werden nur geladen, wenn die Konfigurationsdateien gefunden werden. Siehe hierzu \cmdref{solLoadLanguage}.}
    \end{argument}

\subsection{Farben}

    Dieses Paket definiert einige Farben mittels \cmd{colorlet}. Wird \T{sopra-base} als geladen erkannt, wird versucht passende Farben zu setzen. \imptext{Alle folgenden Farben tragen das Präfix \T{sol@colors@}}: \T{border}, \T{background}, \T{lst@keywordA}, \T{lst@keywordB}, \T{lst@keywordC}, \T{lst@numbers}, \T{lst@literals}, \T{lst@comments} und \T{lst@highlight}, wobei die Bedeutung der einzelnen Farben aufgrund ihres Namens hervorgehen sollte (jeder Sprache steht es frei die gleich zu besprechenden Stile, die
    auf diesen Farben aufbauen, (beliebig) zu verwenden). Beispielsweise kann man die Farbe
    von Kommentaren wie folgt ändern:
\begin{plainlatex}
\colorlet{sol@colors@lst@comments}{orange}
\end{plainlatex}
    Wir erhalten(examplarisch in Java):
    \bgroup
\begin{java}
...
public static void main(...){ /* ... */ }
    \end{java}
    \egroup
 Es ist möglich jede Farbe (nach \T{xcolor}) zu verwenden.
...
\subsection{Stile}
    ...
    Stile sollten bei einer kompletten Neudefintion über \cmd{sol@list@define@styles} 
    ...
    Es gibt die folgenden Stile: \T{keywordA}, \T{keywordB}, \T{keywordC}, \T{keywordD}, \T{numbers}, \T{linenumbers}, \T{literals}, \T{comments}, \T{highlight} und \T{basic}, wobei letzterer jedem Stil vorangestellt wird. Die neudefinition eines Stils sollte
    nicht leichtfertig getätigt werden und benltigt deswegen etwas mehr Aufwand. Hier ein Beispiel:
...
\begin{command}{RegisterLanguage}{\manArg{Sprache}\manArg{LanguageName}}
...
\end{rubberduck}
    \notetext{Die Befehle wie \cmdref{c<Sprache>}, also \cmd{crubberduck} funkionieren natürlich auch: \crubberduck{Duck jens}.}
\end{command}
...
auf \say{Blank} zurückgesetzt werden. \notetext{Hinweis: Die Definition der Sprachen verläuft, Überraschung, analog zu Lilly (\url{https://github.com/EagleoutIce/LILLY}, nur die gefordete Dateinennung und die weiteren Möglichkeiten unterscheiden sich.), also können auch
...
\begin{multicols}{4}
    ...
    \end{description}
\end{multicols}
\notetext{Hinweis: Ja, \T{:c:} expandiert zu nichts. Es kann als Trenner verwendet werden.}
